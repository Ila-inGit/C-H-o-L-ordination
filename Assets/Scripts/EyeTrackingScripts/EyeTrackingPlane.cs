...
public class EyeTrackingPlane : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((CoreServices.InputSystem != null) && (CoreServices.InputSystem.EyeGazeProvider != null) &&
            ...
            CoreServices.InputSystem.EyeGazeProvider.GazeTarget == this.gameObject)
        { //if the gaze target is the eyePlane
            ...
            "-1", gameObject.scene.name, headOrigin, headDir, gazeOrigin, gazeDir, eyeOrigin, eyeDir, eyeHitPos, SceneChangerManager.Instance.isMusicSynch(),
            SceneChangerManager.Instance.isRhythmSynch(), SceneChangerManager.Instance.isRhythmNotSynch(), SceneChangerManager.Instance.isMusicNotSynch(), SceneChangerManager.Instance.getDifficulty()));
